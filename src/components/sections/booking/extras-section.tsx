...
export function ExtrasSection({
  ...
  return (
    <section>
      <h2 className="font-manrope text-base font-semibold leading-none tracking-tight-2 text-[#141543]">Extras</h2>

      <div className="mt-5 space-y-3">
        {extras.map((extra) => {
          const state = selectedExtras[extra.id] || { enabled: false, quantity: 1 };

          return (
            <div
              key={extra.id}
              className="flex items-center justify-between rounded-lg border border-[#f4f4f4] px-4 py-3"
            >
              <div className="flex items-center gap-3">
                {/* Icon */}
                <Image
                  src="/icons/booking/add-extra.svg"
                  alt="Extra"
                  width={24}
                  height={24}
                />
                ...
                {state.enabled ? (
                  <div className="flex items-center">
                    <div className="flex items-center rounded-lg bg-primary py-1 pl-3 pr-1">
                      ...
                      <button
                        type="button"
                        onClick={() => onQuantityChange(extra.id, state.quantity + 1)}
                  ...
                  <button
                    ...
                    className="relative h-[27px] w-12 rounded-lg bg-[#9d9ca3] transition-colors"
                  >
